        ...
        <v-app-bar scroll-behavior="fade-image inverted" density="prominent"  class="mx-auto d-flex align-center justify-center">
            ...
            <template v-slot:append>
                ...
                <v-chip size="large" :color="color" label>
                    Usuario
                    <v-icon end icon="mdi-account-group-outline"></v-icon>
                </v-chip>
            </template>
        </v-app-bar>
        <v-app-bar :color="color" elevation="1" height="18"></v-app-bar>
        <v-navigation-drawer v-model="drawer" floating permanent color="rgb(0, 0, 0, 0)" width="400">
            <v-card rounded="lg" elevation="3" width="390" class="mt-4 ml-1">
                <v-list density="compact">
                    <v-list-item title="Usuario" subtitle="Menú">
                        <template v-slot:prepend>
                ...
                <v-list rounded="lg" shaped>
                    <template v-for="link in links" :key="link.id">
                        <v-list-group v-if="link['submenu']" :value="link['id']" :key="link['id']" :color="color">
                            <template v-slot:activator="{ props }">
                            ...
                            <v-list-item v-for="link2 in link['submenu']" :key="link2['subId']" :to="link2['subLink']" :value="link2['subId']" @click="cambiaColor(link2['subId'])" :color="color" rounded="xl">
                                <template v-slot:prepend>
                                    <v-icon left>
                                        {{ link2['icon'] }}
                                    </v-icon>
                                </template>
                                <v-list-item-title class="text-wrap"> {{ link2['subnombre'] }}</v-list-item-title>
                            </v-list-item>
                        </v-list-group>
                        <v-list-item @click="cambiaColor(link['id'])" :to="link['link']" :value="link['id']"  link :color="color" rounded="xl" v-else>
                            <template v-slot:prepend>
                                <v-icon left>
                                    {{ link['icon'] }}
                                </v-icon>
    ...
    export default {
        ...
        data: () => ({
            ...
            /*links: <Array<Menu>>[
                ...
                { id: 98, nombre: 'Acreditación  las Represetaciones de los Partidos Políticos y Candidaturas sin partido', icon: 'mdi-card-account-details-star', link:'',
        ...
        mounted(){
            ...
            this.idDistrito = localStorage.idDistrito;
            //Modo oscuro o claro
            if(localStorage.lightBlack){
                //this.$vuetify.theme.global.name = localStorage.lightBlack;
            }
